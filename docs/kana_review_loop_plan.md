...
「し」失敗→再発防止: **スカラー帯より視覚を優先**。委譲時に必ず `width hw ∈[8,40]`・入口 hw≤18・最大 hw≤38 を制約に書く（帯だけ追うと矩形頭で崩壊した）。

**中断耐性（必須）**: 長時間 Task 1本（≥5–10分・多数 regen）は UI/ユーザー中断で落ちる。
対策:
1. 委譲は **短ターン（≤2 iter）** に分割。各ターン終了時に YAML が valid であること
2. 計測正本はローカル `scripts/kana_fit_step.py`（`proofs/review/<gid>/band_fit.jsonl` に追記）。再開はログ末尾を見て続ける
3. spine y は UPM 内（0–1000）。出口 hw は先細り（≤5）。途中状態のまま放置しない
