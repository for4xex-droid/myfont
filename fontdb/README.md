...
# T7 は engine の依存（pathops/fontmake）が必要:
../engine/.venv/bin/python scripts/06_ingest_prototype.py
```
...
- `output/t7_bridge/`（T7 OTF + report）

## 状態（2026-08-09）

- T0〜T6 実装済み・実行確認済み（5書体、glyph 60/60 ok、san_uroko 5/5 ok）
- T7: `engine.bridge`＋`scripts/06_ingest_prototype.py` で一時フォント化経路が通る（classic/product_r1）
